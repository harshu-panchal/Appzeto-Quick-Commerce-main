...
function validatePaymentEligibility(order, userId) {
  if (!order) {
  ...
  if (
    ...
    const err = new Error("Payment is not allowed for this order state");
    err.statusCode = 409;
    throw err;
  }
...
function getPayableAmountPaise(order) {
  const amountRupees = Number(order?.paymentBreakdown?.grandTotal ?? order?.pricing?.total ?? 0);
  if (!Number.isFinite(amountRupees) || amountRupees <= 0) {
    const err = new Error("Unable to determine payable amount for this order");
    err.statusCode = 400;
    throw err;
  }
...
async function resolveOrderFromReference(orderRef) {
...
export async function createPaymentOrderForOrderRef({
  ...
  const order = await resolveOrderFromReference(orderRef);
  ...
  const amountPaise = getPayableAmountPaise(order);
  const currency = String(order?.paymentBreakdown?.currency || "INR").toUpperCase();
  const attemptCount = (await Payment.countDocuments({ order: order._id })) + 1;
